...
5. **copy + design** — Write, as JSON in your head: `HEADLINE` (short, editorial — not "Welcome to X"), `HERO_COPY` (1 sentence), `MANIFESTO` (1 punchy "why now" sentence; you may wrap 3-4 key words in `<b>...</b>`), 3× `FEATURE_TITLE`+`FEATURE_BODY`, `CTA_TEXT`, `SIGNUP_HEADLINE`, `SIGNUP_PROMPT`. **Then LOAD the `web-design` skill** (`cat /opt/data/skills/design/web-design/SKILL.md`) and run its 8-step method to lock a **mood pair → layout archetype → Google-Fonts type pairing (never Fraunces+Inter twice; never Inter/Space-Grotesk as display) → derived OKLCH palette + one `ACCENT`**. Write these design decisions down. Report `copy done`.
6. **assets (premium — each degrades, never blocks CORE):**
   - **logo:** `bash $SK/scripts/logo_gen.sh "minimalist flat-vector logo for <brand>, <one visual idea from the brief>, <accent> on off-white, centered, no text" $WORK/logo.png`. There is no separate `logo` DAG node: on `LOGO_UNAVAILABLE` continue silently (no logo); otherwise mention the logo outcome in the `engineer` note.
   - **images (HARD CAP 2 calls/run — budget; no retries on failure):**
     hero (ALWAYS): `bash $SK/scripts/image_gen.sh "cinematic editorial photograph for <brand>: <one atmospheric scene from the brief/vertical>, moody, dark-toned, <accent> color notes, shallow depth of field, no text, no words, no logos, no watermarks" $WORK/hero.png 1536x1024`.
     showcase (ONLY if the vertical is visual — fashion/apparel, food/beverage, beauty/cosmetics, retail/product, travel/hospitality, interiors): `bash $SK/scripts/image_gen.sh "premium product photograph for <brand>: <hero product or signature look>, centered subject, studio quality, clean backdrop, <accent> accent, no text, no logos, no watermarks" $WORK/showcase.png 1024x1024`.
     No separate DAG node: on `IMAGE_UNAVAILABLE` continue silently — the hero keeps its shader/CSS gradient and the showcase section hides itself. NEVER a third image call, never a retry.
...
7. **engineer** — Build a **BESPOKE** premium `index.html` that executes the web-design decisions from step 5 (the chosen archetype, type pairing, and OKLCH palette) — every brand should look different. The base template (`$SK/templates/landing-page/index.html`) is a **floor/fallback**, not a mold: depart from its layout to fit the archetype. Write the full self-contained page to `$WORK/index.html` **via the terminal tool** (a `python3`/heredoc write — NEVER `write_file`). It MUST include: the derived Google-Fonts `<link>`, the derived palette + `ACCENT`, the logo (`<img src="logo.png">` if generated), the hero image (`{{HERO_IMAGE}}` → relative path `hero.png` if `$WORK/hero.png` exists, else the EMPTY string — the hero then falls back to the shader gradient; the photo layers UNDER the dark hero overlay, never lighten that overlay), the showcase image (`{{SHOWCASE_IMAGE}}` → relative path `showcase.png` if `$WORK/showcase.png` exists, else the empty string or drop the showcase section entirely) — always RELATIVE paths, deploy_page.sh ships the whole $WORK dir so `hero.png`/`showcase.png` resolve on the live site — the voice ad (`<audio src="ad.mp3">` if generated), a working email **signup form** POSTing to `$CONVEX_LEAD_URL` (`{email, sourceUrl, consent:true}`), and a **"Buy the launch kit" button** linking to the Dodo `CHECKOUT_URL`. If any derivation is uncertain, fall back to filling the base template with the derived fonts+palette — still distinct, never blank. Report `engineer done`.
8. **publish** — `bash $SK/scripts/deploy_page.sh "<winning name>" $WORK` — this deploys the whole dir (index.html + logo.png + hero.png + showcase.png + ad.mp3) in one shot. Capture the `DEPLOYED https://...pages.dev` URL. Report `publish done "<live URL>"`. **After a successful publish** you MAY apply the **web-motion** skill (`cat /opt/data/skills/design/web-motion/SKILL.md`) to layer GSAP hero/scroll reveals onto `$WORK/index.html`, then re-run `deploy_page.sh` — a premium upgrade only: it is degrade-safe (CDN/JS-fail-safe, respects prefers-reduced-motion) and must NEVER block or delay the live URL.
9. **qa** — Report `qa started`. Verify the live URL, tolerating fresh-subdomain propagation: `for i in 1 2 3 4 5; do C=$(curl -s -o /dev/null -w '%{http_code}' "<live URL>"); [ "$C" = 200 ] && break; sleep 5; done`. If still not 200, re-run `deploy_page.sh "<winning name>" $WORK` **once** (deterministic — it redeploys to the SAME URL) and re-check. Report `qa done` when 200, then `deliver done "<live URL>"`.
...
# Verification
Vertical set; a real, premium `*.pages.dev` site is live and 200 with logo + (if available) hero/showcase imagery + voice ad + a Dodo checkout link; every stage reported; CORE never blocked by a premium failure.
